    ...
    : window(win), titleFont(titleFont), bodyFont(bodyFont), soundManager(soundManager)
{
    ...
    soundManager.playMusic("BeginMusic.ogg");
}
    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
            ...
            if (playButton.getGlobalBounds().contains(mousePos)) {
                soundManager.stopMusic();  // Stop background music when entering gameplay
                soundManager.playSound("button_click.wav");  // Play click sound
